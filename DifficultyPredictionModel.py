...
def predict_word_difficulty(new_word):
    ...
    if len(new_word) == 1 or new_word.lower() in easy_words or new_word.isdigit():
        return "easy"
...
def predict_paragraph_difficulty(paragraph):
    ...
    difficult_words = {word for word in words if predict_word_difficulty(word) == "hard"}
    return list(difficult_words)    

# Get the paragraph from stdin (from Node.js)
paragraph = sys.stdin.read().strip()

# Predict difficult words and print as JSON
try:
    difficult_words = predict_paragraph_difficulty(paragraph)
    print(json.dumps(difficult_words))  # Output as JSON
except Exception as e:
